...
//		var_dump($value);
		if(!preg_match("/@/",$text)){
			...
			$sql="INSERT INTO Tweet(Data) VALUES(\"".$text."\")";
			echo $sql."\n";
			db_query($sql,$link);
			...
			//	if(($n->getId())<=68){
					...
					$sql="INSERT INTO POSpeech_".$n->getId()."_db(Data) VALUES(\"".$word."\");";
					db_query($sql,$link);
					$junban[$i%3]=$n->getId();
					if($i==2){
						$sql="INSERT INTO Junban(j0,j1,j2) VALUES("."-1".",".$junban[1].",".$junban[2].");";
						db_query($sql,$link);
						$junban=array(-1,-1,-1);
					}elseif($i!=0 && !($i%3)){
						$sql="INSERT INTO Junban(j0,j1,j2) VALUES(".$junban[0].",".$junban[1].",".$junban[2].");";
						db_query($sql,$link);
						$junban=array(-1,-1,-1);
					}
					$i++;
